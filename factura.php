...
$sql_req = "SELECT nombre_requerimiento AS Requerimiento,descripcion_requerimiento AS Descripcion_Requerimiento_Del_proyecto,costo_requerimiento AS Costo,tiempo_requerimiento AS Tiempo FROM requerimientos_proyectos WHERE id_proyecto = $id";
...
while ($field_info = mysqli_fetch_field($total_proyecto)) {
  $pdf->Cell(43, 6, $field_info->name, 1);
};
...
while ($rows = mysqli_fetch_assoc($total_proyecto)) {
  ...
  foreach ($rows as $column) {
    $pdf->Cell(43, 6, '$', 1);
    $pdf->Cell(43, 6, $column, 1,);
    $pdf->Image('images/Barcode.jpg', 90, 235, 25);
    $pdf->Ln();
    $pdf->Cell(185,70, $numero_aleatorio, 20, 20, 'C');
  }
}
